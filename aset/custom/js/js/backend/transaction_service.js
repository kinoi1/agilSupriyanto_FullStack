...
var save_method = "add";
var table;
var url_list    = host + "transaction_service/list_data/";
var url_edit    = host + "transaction_service/edit/";
var url_hapus   = host + "transaction_service/delete/";
var url_simpan  = host + "transaction_service/save/";
var url_approve = host + "transaction_service/approve_data/";
var addressno   = 0;
var contactno   = 0;
var modul       = "";
var app         = "";
...
function tambah(modul){
    ...
    $("#form [name=TransactionServiceID]").val("");
    ...
    $.ajax({
        ...
        success: function(json){
            ...
            if(json.Status){
                ...
                NextID = a.NextID;
                PrevID = a.PrevID;
                $("#form [name=TransactionServiceID]").val(a.TransactionServiceID);
                ...
                $("#form [name=VehicleName]").val(a.VehicleNo);
                ...
                $("#form [name=SparepartDate]").val(a.SparepartDate);
                ...
                if(method_before == "view"){
                    ...
                    $("#form [name=TransactionServiceID], #form [name=Remark], #add-attachment").attr("disabled",false);
                    $("#form [name=TransactionServiceID], #form [name=Remark]").removeClass("text");
                }
...
function save(element)
{
    ...
    if(count_save == 0){
        ...
        method = dt.method;
        url = host;
        url = host + 'transaction_service/save/'+save_method;
        $.ajax({
            url : url,
            type: "POST",
            data: $('#form').serialize(),
            ...
            success: function(json)
            {
                if(json.Status){
                    ...
                    if(method == "close"){
                        ...
                        save_method = "update";
                        $("#form [name=TransactionServiceID]").val(json.TransactionServiceID);
                        $("#form [name=Code]").val(json.Code);
                        $("#form [name=Codex]").val(json.Code);
                        // div_form('change_id');
                        edit('<span data-id="'+json.TransactionServiceID+'" data-method="edit"></span>');
                    }
                    reload_table();
...
function approve_data_save(element){
    ...
    $.ajax({
        url : host+'transaction_service/approve_data/',
        type: "POST",
        data: data_post,
        dataType: "JSON",
        success: function(json){
...
function add_data_detail(method,element){
    ...
    if(method == "add_new"){
        ...
        // i_search = '<a href="javascript:;" class="btn btn-xs btn-white pointer" onclick="modal_vehicle(this)" data-modul="transaction_service" data-rowid="'+rowid+'"><i class="fa fa-search"></i></a> ';
    ...
    if(method == "empty"){
        ...
        item += '<td>'+Name+'</td>';
        item += '<td>'+Remark+'</td>';
        item += '<td class="text-center">'+Qty+'</td>';
        item += '<td class="text-center">'+Unit+'</td>';
        ...
        item += '<td><input name="DetailName[]" class="item-name" type="text" maxlength="30" data-rowid="'+rowid+'" onkeyup="calculation_total_price()" style="width:'+td_width+'"></td>';
        item += '<td><input name="DetailRemark[]" class="item-remark" type="text" maxlength="30" data-rowid="'+rowid+'" style="width:'+td_width+'"></td>';
        item += '<td><input name="DetailQty[]" class="item-qty text-center" type="text" maxlength="4" onkeyup="calculation_total_price()" oninput="isNumber(this)" data-rowid="'+rowid+'" value="'+Qty+'" style="width:60px;"></td>';
        item += '<td><input name="DetailUnit[]" class="item-unit" type="text" maxlength="30" data-rowid="'+rowid+'" style="width:'+td_width2+';"></td>';
        item += '<td class="text-right"><input name="DetailPrice[]" class="item-price" type="text" maxlength="20" onkeyup="calculation_total_price()" oninput="isNumber(this)" data-format="duit" data-rowid="'+rowid+'" style="width:'+td_width2+';"></td>';
...
function calculation_total_price(){
    ...
    $(".table-data-detail tfoot .item-total1").text(total1);
    $(".table-data-detail tfoot .item-total3").text(number_format(total3));
    $("#form [name=TotalPrice]").val(number_format(total3));
}
...
function ChangeType(element){
    ...
    if(value == 1){
        $(".v_tipe").removeClass("col-sm-12");
        $(".v_tipe").addClass("col-sm-6");
        $(".v_dealer").removeClass("sembunyi");
    } else {
        $(".v_tipe").addClass("col-sm-12");
        $(".v_tipe").removeClass("col-sm-6");
        $(".v_dealer").addClass("sembunyi");
    }
}