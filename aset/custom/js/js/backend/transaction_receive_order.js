...
var save_method = "add";
var table;
var url_list = host + "transaction_receive_order/list_data/";
...
const idScanner = function () {
    let urlParam = url.split('?');
    if (urlParam.length > 1) {
        let queryString = new URLSearchParams(urlParam[1]);
        for (let pair of queryString.entries()) {
            if (pair[0] == 'id') {
                let element = `<a href="javascript:void(0)" title="View Data" onclick="edit(this)" data-id="${pair[0]}" data-method="view">Lihat</a>`;
                edit(element);
            }
        }
    }
    ...
    $.ajax({
        ...
        success: function (json) {
            ...
            if (json.Status) {
                ...
                $("#BookingOrder").prop("checked", a.BookingOrder == 1);
                changeBooking();
                if (a.BookingOrder == 1) {
                    $("#BookingDate").val(a.BookingDate);
                    $("#OrderTime").val(a.OrderTime);
                } else {
                    $("#BookingDate, #OrderTime").val("");
                }
                console.log(a.TransactionMembershipID);
                if (a.TransactionMembershipID > 0) {
                    $("#Membership").prop('checked', true).trigger("change");
                    $("#TransactionMembershipID").val(a.TransactionMembershipID).trigger("change");
                    let membershipLabel = $("#TransactionMembershipID").find(":selected").html();
                    $("#TransactionMembershipName").val(membershipLabel);
                } else {
                    // $("#TransactionMembershipName").val("");
                    // $("#TransactionMembershipID").val("none").trigger("change");
                    $("#Membership").prop('checked', false).trigger("change");
                }
                if (parseFloat(a.ReductionPrice) > 0) {
                    $("#ReductionPrice").val(a.ReductionPrice);
                    $("#f_reduction").show();
                } else {
                    $("#f_reduction").hide();
                }
                ...
                if (parseFloat(a.ReductionPrice) > 0) {
                    calculateTarif(a.TotalPriceTxt, a.ReductionPrice);
                }
...
function calculateTarif(price, reduction) {
    reduction = parseFloat(reduction.replaceAll(',', ''));
    price = parseFloat(price.replaceAll(',', ''));
    price = price - reduction;
    $(".table-data-detail tfoot .item-total").text(number_format(price));
}
...
function save(element) {
    ...
    if (count_save == 0) {
        ...
        dt = $(element).data();
        method = dt.method;
        url = host + 'transaction_receive_order/save/' + save_method;
        $.ajax({
            url: url,
            type: "POST",
            data: $('#form').serialize(),
...
function approve_data_save(element) {
    ...
    $.ajax({
        url: host + 'transaction_receive_order/approve_data/',
        type: "POST",
        data: data_post,
        dataType: "JSON",
        success: function (json) {
...
const changeBooking = () => {
    $("#BookingOrder").is(":checked") ? $("#BookingDate, #OrderTime").parent().parent().parent().show(300) : $("#BookingDate, #OrderTime").parent().parent().parent().hide(300);
    $("#BookingDate").datepicker("setDate", CurrentDate);
    $("#OrderTime").val(thisTime);
}

const changeMembershipCode = () => {
    const e = $("#TransactionMembershipID");
    const trMembershipID = e.val();
    if (trMembershipID == "none") {
        return;
    }
    const a = e.find(":selected").data();
    $.ajax({
        url: host + "membership_transaction/edit/" + trMembershipID,
        type: "POST",
        dataType: "JSON",
        data: data_post,
        success: function (json) {
            if (json.Status) {
                let a = json.data;
                $("#VendorID").val(a.VendorID).trigger("change");
...
const resultState = (data, container) => {
    if (data.element) {
...
const changeMembership = () => {
    $("#VendorID").val("none").trigger("change");
    const showNhideCol = `
    .frame_no, 
    .machine_no, 
    .type_member, 
    ...
    $(showNhideCol)
        .hide(300);
    $('.vendor-opt').removeClass('optInvisible');

    if ($("#Membership").is(":checked")) {
        $('.vendor-opt').addClass('optInvisible');
        $('.membership').removeClass('optInvisible');
        $(showNhideCol)
            .show(200);
    }
...
const changeVendor = (e) => {
    GetPPNVendor(e);
    // let selected = $("#VendorID").find(":selected");
    // let mmid = 0;
    // if (selected.hasClass("membership")) {
    //     mmid = selected.data("mmid");
    // }
    // autoFillVehicle(mmid);
}

// const autoFillVehicle = (mID) => {
//     if (mID == 0) {
//         $("#BrandID").val("none").trigger("change");
//         $("#TypeID").val("none").trigger("change");
//         $("#VehicleNo").val("");
//         $("#FrameNo").val("");
//         $("#MachineNo").val("");
//         $("#Color").val("");
...
//     }
//     $.ajax({
//         url: host + 'membership_transaction/edit/' + mID,
//         type: "GET",
//         dataType: "JSON",
//         success: function (json) {
//             if (json.Status) {
...
//         },
//         error: function (jqXHR, textStatus, errorThrown) {
//             toastr.error("Terjadi kesalahan gagal mengambil data kendaraan", "Information");
//             console.log(jqXHR.responseText);
//         }
//     });
// }