...
var save_method = "add";
var table;
var url_list    = host + "transaction_vehicle_exchange/list_data/";
var url_edit    = host + "transaction_vehicle_exchange/edit/";
var url_hapus   = host + "transaction_vehicle_exchange/delete/";
var url_simpan  = host + "transaction_vehicle_exchange/save/";
var url_approve = host + "transaction_vehicle_exchange/approve_data/";
var addressno   = 0;
var contactno   = 0;
var modul       = "";
var app         = "";
...
function tambah(modul){
    ...
    div_form("open");
    $("#form [name=TransactionVehicleExchangeID]").val("");
    ...
    $.ajax({
        ...
        success: function(json){
            ...
            if(json.Status){
                ...
                $("#form [name=TransactionVehicleExchangeID]").val(a.TransactionVehicleExchangeID);
                $("#form [name=Code]").val(a.Code);
                $("#form [name=Codex]").val(a.Code);
                $("#form [name=VehicleSendCode]").val(a.VehicleSendCode);
                ...
                if(method_before == "view"){
                    ...
                    $("#form [name=TransactionVehicleExchangeID], #form [name=Remark], #add-attachment").attr("disabled",false);
                    $("#form [name=TransactionVehicleExchangeID], #form [name=Remark]").removeClass("text");
                }
...
function save(element)
{
    ...
    if(count_save == 0){
        ...
        method = dt.method;
        url = host;
        url = host + 'transaction_vehicle_exchange/save/'+save_method;
        $.ajax({
            url : url,
            type: "POST",
            data: $('#form').serialize(),
            ...
            success: function(json)
            {
                if(json.Status){
                    ...
                    if(method == "close"){
                        ...
                        save_method = "update";
                        $("#form [name=TransactionVehicleExchangeID]").val(json.TransactionVehicleExchangeID);
                        $("#form [name=Code]").val(json.Code);
                        $("#form [name=Codex]").val(json.Code);
                        // div_form('change_id');
                        edit('<span data-id="'+json.TransactionVehicleExchangeID+'" data-method="edit"></span>');
                    }
                    reload_table();
...
function approve_data_save(element){
    ...
    data_post = {
        TransactionVehicleExchangeID : id,
        ApproveStatus : status,
        ApproveRemark : remark
    }
    $.ajax({
        url : host+'transaction_vehicle_exchange/approve_data/',
        type: "POST",
        data: data_post,
        dataType: "JSON",
        success: function(json){
