...
var save_method = "add";
var table;
var url_list    = host + "transaction_survey_result/list_data/";
var url_edit    = host + "transaction_survey_result/edit/";
var url_hapus   = host + "transaction_survey_result/delete/";
var url_simpan  = host + "transaction_survey_result/save/";
var addressno   = 0;
var contactno   = 0;
var modul       = "";
var app         = "";
...
function tambah(modul){
    ...
    div_form("open");
    $("#form [name=TransactionSurveyResultID]").val("");
        ...
        success: function(json){
            ...
            if(json.Status){
                ...
                if(method_before == "view"){
                    ...
                    $("#form [name=TransactionSurveyResultID],#form [name=Remark], #add-attachment").attr("disabled",false);
                    $("#form [name=TransactionSurveyResultID],#form [name=Remark]").removeClass("text");
                }
...
function save(element)
{
    ...
    if(count_save == 0){
        ...
        method = dt.method;
        url = host;
        url = host + 'transaction_survey_result/save/'+save_method;
        $.ajax({
            url : url,
            type: "POST",
            data: $('#form').serialize(),
            ...
            success: function(json)
            {
                if(json.Status){
                    ...
                    if(method == "close"){
                        ...
                        save_method = "update";
                        $("#form [name=TransactionSurveyResultID]").val(json.TransactionSurveyResultID);
                        $("#form [name=Code]").val(json.Code);
                        // div_form('change_id');
                        edit('<span data-id="'+json.TransactionSurveyResultID+'" data-method="edit"></span>');
                    }
                    reload_table();
...
function add_data_detail(method,element){
    ...
    if(method == "empty"){
        ...
        input_h  += '<input type="hidden" name="DetailQty[]" class="item-qty" value="'+Qty+'">';
        item = '<tr class="item '+rowid+'">';
        item += '<td>'+i_alert+'<div class="btn-group btn-xs">'+i_search+i_remove+'</div>'+input_h+'</td>';
        item += '<td class="item-name-txt">'+Name+'</td>';
        // item += '<td><input name="DetailQty[]" value="'+Qty+'" class="item-qty" type="text" maxlength="17" onkeyup="calculation_total_price()" oninput="isNumber(this)" data-rowid="'+rowid+'" style="width:'+td_width+';"></td>';
        item += '<td class="item-unit-txt">'+Unit+'</td>';
        item += '<td class="item-price-txt text-right">'+PriceTxt+'</td>';
